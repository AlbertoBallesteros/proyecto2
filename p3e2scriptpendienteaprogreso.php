<?php
$tarea = $_POST['pendienteaprogreso'];
...
if($pos){
    ...
    $fichero = fopen("pendientes.txt", "r+b");
    ...
    //Si la tarea existe abrimos el archivo, escribimos las tareas ordenadas con el array
        ...
        $fichero = fopen("enprogreso.txt","a+b");
        while($a = fgets($fichero)){
            $tareas = explode(";", $a);
            $id = $tareas[0];
        }
    ...
    if($z == 0){
        ...
        fwrite($fichero, $tareas[0]+1 . ";" . $tarea . "\n");
        fclose($fichero);

        echo "La tarea " . $tarea . " ha pasado de las tareas pendientes a las tareas en progreso.";
    }
}
